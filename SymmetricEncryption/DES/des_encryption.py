...
args = sys.argv
if (len(args) < 4):
    print "$ python des_encryption.py <password> <input file> <output file>"
    sys.exit(1)
...
if __name__ == '__main__':
    ...
    if (len(key) < 8):
        key = key.ljust(8)
    ifile = open(str(args[2]), 'r')
    plaintext = ifile.readline()
    ifile.close()
    if len(plaintext) % 8 != 0:
        plaintext = plaintext.ljust(int(math.ceil(float(len(plaintext)) / 8)) * 8 )

    d = des()
    ciphertext = d.encrypt(key,plaintext)

    ofile = open(str(args[3]), 'w')
